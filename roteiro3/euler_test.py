import unittest
from Grafos.roteiro3.grafo_adj_nao_dir import Grafo
...
class TestEuler(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mod_impar = Grafo([], [])
        for i in ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H']:
            self.mod_impar.adiciona_vertice(i)
        for i in ['A-C', 'A-C', 'A-E', 'A-F', 'A-G', 'A-H', 'B-D', 'B-G', 'C-H', 'C-H', 'D-E', 'D-F', 'E-F', 'F-G',
                  'G-H']:
            self.mod_impar.adiciona_aresta(i)

        self.mod_par = Grafo([], [])
        for i in ['A', 'B', 'C', 'D', 'E', 'F', 'G']:
            self.mod_par.adiciona_vertice(i)
        for i in ['A-B', 'A-C', 'A-F', 'A-G', 'B-C', 'B-F', 'B-G', 'C-D', 'C-E', 'C-F', 'C-G', 'D-E', 'F-G']:
            self.mod_par.adiciona_aresta(i)
    ...
    def test_qual_caminho_euler(self):
        self.assertEqual(self.konigsberg_mod.qual_caminho_euler(),['T-R', 'R-M', 'M-T', 'T-M', 'M-B', 'B-R', 'R-M', 'M-B'])
        self.assertEqual(self.g_c_e.qual_caminho_euler(),['A-B', 'B-C'])
        self.assertEqual(self.mod_impar.qual_caminho_euler(),['D-E', 'E-F', 'F-G', 'G-H', 'H-A', 'A-C', 'C-H', 'H-C', 'C-A', 'A-F', 'F-D', 'D-B', 'B-G', 'G-A', 'A-E'])
        self.assertEqual(self.mod_par.qual_caminho_euler(),['A-B', 'B-C', 'C-D', 'D-E', 'E-C', 'C-F', 'F-G', 'G-A', 'A-C', 'C-G', 'G-B', 'B-F', 'F-A'])
