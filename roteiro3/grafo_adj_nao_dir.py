...
class Grafo:
    ...
    def caminho_euleriano(self):
        ...
        if (self.eh_conexo) and (cont == 2 or cont == 0):
            return True
        else:
            return False

    def qual_caminho_euler(self):
        vez=''
        aresta = self.M
        vertice = self.N
        impar=[]
        caminho=[]
        zerou=False
        for i in range(len(vertice)): #verifica de onde deve começar o caminho
            if self.grau(vertice[i])%2!=0:
                vez=vertice[i]
                break
            vez=vertice[0]
                        ...
                        if aresta[a][v]!='-' and aresta[a][v]!=0:
                            if self.grau(vertice[v])==1:
                                ...
                                for j in range(len(aresta)):
                                    if aresta[a][j]!='-' and aresta[a][j]!=0:
                                        cone+=1
                                if cone>1:
                                    continue
                            caminho+=['{}-{}'.format(vertice[a],vertice[v])]
                            aresta[a][v]-=1
                            vez=vertice[v]
                        elif v==len(aresta)-1: #verifica coluna
                            for c in range(len(aresta)):
                                if aresta[c][a]!='-' and aresta[c][a]!=0:
                                    if self.grau(vertice[c]) == 1:
                                        cone = 0
                                        for j in range(len(aresta)):
                                            if aresta[j][a] != '-' and aresta[j][a] != 0:
                                                cone += 1
                                        if cone > 1:
                                            continue
                                    caminho += ['{}-{}'.format(vertice[a], vertice[c])]
                                    aresta[c][a] -= 1
                                    vez = vertice[c]
                                    break
            uns=0
            for a in range(len(aresta)): #verifica se a matriz zerou
                for v in range(len(aresta)):
                    if aresta[a][v] != '-' and aresta[a][v] != 0:
                        uns+=1
            if uns==0:
