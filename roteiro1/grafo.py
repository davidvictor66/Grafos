...
class Grafo:
    ...
    def vertices_nao_adjacentes(self):
        ...
        for i in range(len(vertice)):
            for a in range(len(vertice)):
                existe1=self.existeAresta('{}-{}'.format(self.N[i],self.N[a]))
                existe2=self.existeAresta('{}-{}'.format(self.N[a],self.N[i]))
                if existe1==False and existe2==False:
                    lista+=['{}-{}'.format(self.N[i],self.N[a])]
        return lista

    def grau(self,v):
        arestas=list(self.A.keys())
        contador=0
        for i in range(len(arestas)):
            if v in self.A.get(arestas[i]):
                contador+=1
        return contador
    ...
    def eh_completo(self):
        ...
        for i in range(len(vertices)):
            for a in range(len(vertices)):
                if '{}-{}'.format(vertices[i],vertices[a]) in aresta or '{}-{}'.format(vertices[a],vertices[i]) in aresta:
                    saida=True
                elif a!=i:
                    return False
        return saida
